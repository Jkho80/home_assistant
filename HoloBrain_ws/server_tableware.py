...
LOGGER = logging.getLogger("organize_tableware")
logging.basicConfig(
    level=logging.INFO,
...
class ServerConfig:
    ...
    expected_instruction: Optional[str] = "Shake the bottle with proper arm."
...
def decode_request(request_data) -> MultiArmManipulationInput:
    ...
    instruction = request_data.get("instruction", "Shake the bottle with proper arm.")
...
class HBMHoloBrainRunner:
    def __init__(self, cfg: ServerConfig):
        ...
        if not fixed_text_dir:
            auto_dir = os.path.join(os.path.dirname(os.path.abspath(cfg.encoder_hbm)), "fixed_text_shake_bottle")
...
def main():
    ...
    parser.add_argument("--server_config", type=str, default="/home/sunrise/HoloBrain_ws/server_tableware.json")
